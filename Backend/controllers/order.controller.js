...
export const getAllOrders = async (req, res) => {
  try {
    ...
    if (!["edu", "pen"].includes(userGroup)) {
      return res.status(403).json({ error: "Access denied for this group" });
    }
...
export const getAllOrderList = async (req, res) => {
  try {
    ...
    // Retrieve all order IDs associated with the provided reference number and user group
    const orders = await Order.find(
      { referenceNumber: refNo, group: userGroup },
