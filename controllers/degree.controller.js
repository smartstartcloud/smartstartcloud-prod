...
export const newDegree = async (req, res) => {
  try {
    ...
    if (currentDegree){
      ...
      const logMessage = `User created Degree ${degreeName} (ID: ${degreeID}) at ${new Date().toISOString()}.`;
...
export const updateDegree = async (req, res) => {
  try {
    ...
    if (currentDegree) {
      ...
      const logMessage = `Degree ${degreeName} (ID: ${degreeID}) was updated at ${new Date().toISOString()}.`;
...
export const deleteDegree = async (req,res)=>{
  try {
    ...
    const logMessage = `Degree with ID ${degreeID} was deleted at ${new Date().toISOString()}.`;
    await createLog({
      req,
      collection: "Degree",
      action: "delete",
...
export const deleteStudentFromDegree = async (req,res)=>{
  ...
  try {
    ...
    const logMessage = `Student with ID ${studentID} was removed from Degree ${degreeID} at ${new Date().toISOString()}.`;
    await createLog({
      req,
      collection: "Degree",
      action: "delete",
